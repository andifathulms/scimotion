...
<KeyTakeaways>
- Binary search halves the search space every step, so it finds any item in a sorted list of $n$ in at most $\lceil \log_2 n \rceil$ comparisons — 20 for a million, 30 for a billion.
...
- It needs sorted data. For pure membership tests a hash table is $O(1)$; choose binary search when you also need order and range queries.
</KeyTakeaways>
