...
const LABEL: Record<Tonicity, string> = {
  hypo: 'Hypotonic — more solute inside → water floods in → cell swells (may burst)',
  iso: 'Isotonic — solute balanced → no net water flow → cell holds its shape',
  hyper: 'Hypertonic — more solute outside → water leaves → cell shrivels',
}
...
export function OsmosisAnimation() {
  ...
  const seedSolutes = useCallback((ton: Tonicity) => {
    ...
    inSoluteRef.current = Array.from({ length: N_IN_SOLUTE }, () => ({
      ...
      frac: Math.sqrt(Math.random()) * 0.82,
    }))
    ...
    <div className="animation-block" ref={ref}>
      <div className="animation-header">
        <span className="animation-label">
          <Play size={13} /> Interactive · Osmosis and tonicity
        </span>
        <button
          onClick={reset}
